...
scripts/hash-pdfs.mjs     stages hashed PDF copies at build    scripts/vercel-config.mjs  postbuild: security headers + monthly cron into .vercel/output/config.json (vercel.json is ignored by the adapter)
src/pages/api/admin/selftest.ts   monthly site check (cron with CRON_SECRET, or the editor's Site check tab); emails SELFTEST_TO_EMAIL via Resend
src/pages/privacy.astro + copy/privacy.json   privacy notice, linked from the footer. Vercel Web Analytics script in Base.astro (prod only).
src/components/OutreachMap.astro   outlined world map of outreach entries. `src/data/places.json` = city → [lat, lon] (add a city here when an entry's city has no pin; the build warns). `src/data/world-map.json` = country/land/lake/state outlines for the world and each zoom region, generated by `node scripts/build-map.mjs` (run by hand after adding a place) from Natural Earth (world-atlas, us-atlas) and OSM lakes (credited on the page); `src/lib/geo.mjs` = projection + cluster/zoom tree shared by both.
```
