...
src/components/OutreachMap.astro   dot-matrix world map of outreach entries. `src/data/places.json` = city → [lat, lon] (add a city here when an entry's city has no pin; the build warns). `src/data/world-map.json` = the dots, generated by `node scripts/build-map.mjs` (run by hand after adding a place; takes a few minutes) from Natural Earth land minus OSM lakes; `src/lib/geo.mjs` = projection + cluster/zoom tree shared by both.
```
